...
Personal email and phone details are omitted from the public profile data, website, and résumé. The supplied LinkedIn profile is linked from the main navigation and footer, with its HTTPS URL centralized in `data/profile.ts`. Both links open in a new tab. A contact form is deferred; no form or delivery service is configured.
